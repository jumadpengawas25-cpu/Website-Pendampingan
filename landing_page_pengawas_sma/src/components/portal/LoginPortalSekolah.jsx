...
export default function LoginPortalSekolah({ schoolSlug }) {
  ...
  const handleSubmit = (e) => {
    ...
    if (result.success) {
      const targetSlug = result.schoolSlug || schoolSlug;
      navigate(`/portal-sekolah/${targetSlug}`);
    } else {
      ...
      <div className="w-full max-w-md bg-surface-container-lowest rounded-2xl shadow-2xl border border-outline-variant overflow-hidden">
        ...
        <div className="px-6 pb-6 text-center">
          <p className="text-label-sm text-on-surface-variant">
            Butuh bantuan? Hubungi pengawas SMA kabupaten.
          </p>
        </div>
      </div>
    </div>
